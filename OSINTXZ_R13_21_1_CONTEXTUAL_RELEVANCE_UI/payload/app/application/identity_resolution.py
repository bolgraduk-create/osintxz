...
def extract_identity_signals(value: Any) -> IdentitySignals:
    """Extract a bounded set of non-secret identity hints from a record or UI row."""
    ...
    if isinstance(value, dict):
        ...
        if title and type_name in {"person", "person_name", "individual", "author", "researcher", "professional", "sole_trader", "natural_person"}:
            signals.names.add(title)
        identifiers = value.get("identifiers")
        if isinstance(identifiers, dict):
            _collect_mapping(identifiers, signals, depth=0)
        for key in ("attributes", "metadata", "identityContext", "_identityContext"):
            _collect_mapping(value.get(key), signals, depth=0)
        return signals

    display = _text(getattr(value, "display_name", ""))
    record_type_obj = getattr(value, "record_type", "") or getattr(value, "entity_kind", "")
    record_type = _norm_key(getattr(record_type_obj, "value", record_type_obj))
    if display and record_type in {"person", "person_name", "individual", "author", "researcher", "professional", "sole_trader", "natural_person"}:
        signals.names.add(display)
    identifiers = getattr(value, "identifiers", None)
    if isinstance(identifiers, dict):
        _collect_mapping(identifiers, signals, depth=0)
    for attr in ("attributes", "metadata"):
        _collect_mapping(getattr(value, attr, None), signals, depth=0)
    return signals
...
def _collect_mapping(value: Any, signals: IdentitySignals, *, depth: int, key: str = "") -> None:
    ...
    if isinstance(value, dict):
        for index, (child_key, child) in enumerate(value.items()):
            ...
            if any(marker in name for marker in _SECRET_MARKERS):
                continue
            _collect_mapping(child, signals, depth=depth + 1, key=name)
        return
    if isinstance(value, (list, tuple, set, frozenset)):
        for child in list(value)[:80]:
            _collect_mapping(child, signals, depth=depth + 1, key=key)
        return
...
def _is_name_key(key: str) -> bool:
    if not key:
        return False
    exact = {
        "name", "full_name", "fullname", "person_name", "display_name",
        "author", "creator", "owner", "officer", "director", "founder",
        "researcher", "contributor", "applicant", "inventor",
    }
    if key in exact:
        return True
    return key.endswith("_name") and not any(
        item in key for item in ("company", "organization", "organisation", "court", "category", "judgment")
    )
