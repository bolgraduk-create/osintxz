...
def main() -> int:
    ...
    try:
        ...
        checks = [
            ...
            (
                "worker emits provider rows",
                (
                    '"osint_provider_rows":'
                    'osint_result.provider_rows'
                    in compact
                ),
            ),
        ]
        ...
        for target_type, goals in expected.items():
            actual = tuple(
                goal.value
                for goal in policy.default_goals(
                    target_type
                )
            )
            checks.append(
                (
                    f"{target_type.value} goals",
                    actual == goals,
                )
            )
        ...
        print(
            "M021.16.7.7.1 "
            "ALL EXISTING UI INTEGRATION AUDIT"
        )
        print("=" * 84)
