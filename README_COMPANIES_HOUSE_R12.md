...
The v3 provider narrows local input-validation handling:
- malformed user-supplied company numbers -> `NOT_SUPPORTED`;
- malformed/unexpected Companies House responses -> `FAILED` with
  `failure_isolated=True`.

This fixes the regression where upstream schema errors were incorrectly
classified as unsupported user queries.
