...
def assess_result_row(row: dict[str, Any]) -> RelevanceAssessment:
    ...
    if kind == "username":
        ...
        if wanted and (identifier_match or title_match or url_match):
            reason = "Username identifier matches" if identifier_match else "Username account URL matches" if url_match else "Username value matches"
            return RelevanceAssessment("relevant", 96.0 if identifier_match else 90.0, (reason,), (), True, True)
        return RelevanceAssessment(
            "low_relevance", 8.0, (),
...
def _username_url_match(wanted: str, value: Any) -> bool:
    ...
    for found in re.findall(r"https?://[^\s<>\"]+", str(value or "")):
        ...
        # Profiles and repositories normally expose the account owner near the
        # beginning of the path. Do not treat arbitrary mentions/query params as
        # ownership evidence.
        if any(_normalize_exact("username", segment) == wanted for segment in segments[:3]):
            return True
    return False
