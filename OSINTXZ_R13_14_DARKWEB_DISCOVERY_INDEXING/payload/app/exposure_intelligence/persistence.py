...
class ExposurePersistenceService:
    ...
    def persist(self, *, case_id: UUID, result) -> ExposurePersistenceResult:
        ...
        for original in federated.records:
            ...
            search_terms = self._safe_search_terms(record)
            description = marker + "\nExposure intelligence observation; identity/ownership is not inferred."
            if search_terms:
                description += "\nIndexed indicators: " + " | ".join(search_terms)
        ...
        return out
    @staticmethod
    def _safe_search_terms(record) -> list[str]:
        """Expose only non-secret indicators to the existing text index."""
        allowed = {
            "email",
            "domain",
            "username",
        ...
        out: list[str] = []
        seen: set[str] = set()
        indicators = (record.attributes or {}).get("indicators") or []
        ...
        for item in indicators:
            ...
            if kind not in allowed or not value or len(value) > 512:
                continue
            key = value.casefold()
            if key in seen:
                continue
