...
def test_maigret_fast_pass_is_bounded_and_non_recursive():
    ...
    for marker in ('"--no-recursion"', '"--no-extracting"', '"--retries", "0"'):
        assert marker in text

    match = re.search(r'"--top-sites"\s*,\s*"(\d+)"', text)
    assert match is not None, "Maigret fast-pass must keep an explicit --top-sites bound."
    top_sites = int(match.group(1))
    assert 1 <= top_sites <= 500, "Maigret fast-pass must remain bounded to at most 500 sites."
