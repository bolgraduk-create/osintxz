...
class DesktopBridge(QObject):
    ...
    @Property("QVariantMap", notify=changed)
    def analysisMediaWorkspace(self) -> dict[str, Any]:
        return self._analysis_media_workspace_payload()

    @Property("QVariantMap", notify=changed)
    def analysisMapWorkspace(self) -> dict[str, Any]:
        return self._analysis_map_workspace_payload()
    ...
    def _analysis_case_evidence(
        self,
        *,
        limit: int = 300,
    ) -> list[Any]:
        """Return a bounded evidence slice for the active investigation."""
        ...
        try:
            ...
            try:
                ...
                LOGGER.debug(
                    "Unable to load analysis evidence fallback",
                    exc_info=True,
                )
                return []
        except Exception:
            LOGGER.debug(
                "Unable to load analysis evidence",
                exc_info=True,
            )
            return []

    @staticmethod
    def _analysis_processing_metadata(
        metadata: dict[str, Any],
    ) -> dict[str, Any]:
        direct = metadata.get(
            "processing_metadata"
    ...
    @classmethod
    def _analysis_extract_gps(
        cls,
        metadata: dict[str, Any],
    ) -> dict[str, Any]:
        processing = (
            cls._analysis_processing_metadata(
                metadata
            )
        )
    ...
    def _analysis_media_record(
        ...
        ocr_data = cls._analysis_result_data(
            image_analysis.get(
                "ocr"
            )
        )
        face_data = cls._analysis_result_data(
            image_analysis.get(
                "faces"
            )
        )
        ...
        if (
            ...
            transcription_data = transcription.get(
                "data"
            )
            if isinstance(
                transcription_data,
    ...
    def _analysis_media_workspace_payload(
        self,
    ) -> dict[str, Any]:
        media_types = {
            "image",
        ...
        for evidence in self._analysis_case_evidence():
            evidence_type = str(
                getattr(
                    getattr(
                        evidence,
            ...
            if evidence_type not in media_types:
                continue

            items.append(
                self._analysis_media_record(
                    evidence
                )
            )
    ...
    @classmethod
    def _analysis_location_coordinates(
        cls,
        entity: Any,
        metadata: dict[str, Any],
    ) -> dict[str, Any]:
        ...
        return {
            "available": (
                normalized_latitude is not None
                and normalized_longitude is not None
            ),
            "latitude": normalized_latitude,
            "longitude": normalized_longitude,
            "altitude": normalized_altitude,
    ...
    def _analysis_case_location_entities(
        self,
        *,
        limit: int = 300,
    ) -> list[Any]:
        ...
        try:
            ...
            LOGGER.debug(
                "Unable to load analysis location entities",
                exc_info=True,
            )
            return []

    def _analysis_map_workspace_payload(
        self,
    ) -> dict[str, Any]:
        markers: list[
            dict[str, Any]
        ...
        for entity in self._analysis_case_location_entities():
            ...
            coordinates = (
                self._analysis_location_coordinates(
                    entity,
                    metadata,
                )
            )
            ...
            if coordinates.get(
                ...
                if marker_key not in seen_marker_keys:
                    seen_marker_keys.add(
                        marker_key
                    )
                    markers.append(
                        row
        ...
        media_payload = (
            self._analysis_media_workspace_payload()
        )
        for item in media_payload.get(
            "items",
            []
        ):
            if not isinstance(
