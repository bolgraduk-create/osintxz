...
class UnifiedInvestigationSearchWorker(QObject):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            results = self._dedupe_result_rows(results)
            ...
            for provider in snap.get("providers", []):
                providers.append(
                    self._provider_row(
                        ...
                        seed=seed,
                        detail=str(provider.get("error") or route.capability),
                    )
                )
            ...
            for provider in list(getattr(search, "provider_results", ()) or ()):
                ...
                providers.append(
                    self._provider_row(
                        ...
                        seed=seed,
                        detail=str(snap.get("error") or query.kind.value),
                    )
                )
                ...
                results.append(
                    {
                        ...
                        "depth": seed.depth,
                        "candidateOnly": bool(row.get("candidateOnly")),
