...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Install OSINTXZ R13.20.2 person-name relevance hotfix"
    )
    ...
    if missing:
        raise SystemExit(
            "R13.20.1 baseline is required before R13.20.2. Missing: "
            + ", ".join(missing)
        )

    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup = root / "storage" / "patch_backups" / f"r13_20_2_{stamp}"
