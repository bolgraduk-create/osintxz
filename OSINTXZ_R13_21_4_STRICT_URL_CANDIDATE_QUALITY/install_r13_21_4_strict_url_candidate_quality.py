...
def verify_baseline(root: Path) -> None:
    ...
    if missing:
        raise SystemExit("R13.21.3 baseline is required. Missing: " + ", ".join(missing))
    ...
    if absent:
        raise SystemExit(
            "R13.21.4 requires installed R13.21.3. Missing markers: "
            + ", ".join(absent)
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Install OSINTXZ R13.21.4 strict URL relevance and candidate quality hotfix"
    )
    ...
    backup = root / "storage" / "patch_backups" / f"r13_21_4_{stamp}"
    ...
    consolidation = (root / "app/application/investigation_result_consolidation.py").read_text(
        encoding="utf-8", errors="ignore"
    )
    persistence = (root / "app/application/unified_persistence_relevance.py").read_text(
        encoding="utf-8", errors="ignore"
    )
    qml = (root / "app/interface/desktop/qml/pages/Search.qml").read_text(
        encoding="utf-8", errors="ignore"
    ...
    markers = (
        ("Returned URL is outside the searched URL/profile scope", relevance),
        ("_url_seed_relation", relevance),
        ("structuredPersonMatch", consolidation),
        ("Identity-review rows live in Identity only", consolidation),
        ("_username_from_account_url", persistence),
        ('title: "Identity Leads"', qml),
        ("root.summary.identityInsufficient", qml),
        ('" review · "', qml),
    )
    absent = [marker for marker, content in markers if marker not in content]
    if absent:
        raise SystemExit("R13.21.4 install verification failed: " + ", ".join(absent))
