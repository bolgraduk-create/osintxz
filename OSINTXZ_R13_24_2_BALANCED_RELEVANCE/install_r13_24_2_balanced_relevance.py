...
def verify_baseline(root: Path) -> None:
    ...
    if missing:
        raise SystemExit("R13.24.1 baseline required. Missing: " + ", ".join(missing))

    worker = (root / "app/interface/desktop/workers/unified_investigation_search_worker.py").read_text(
        encoding="utf-8", errors="ignore"
    )
    maigret = (root / "app/osint/connectors/maigret_connector.py").read_text(
        encoding="utf-8", errors="ignore"
    )
    ...
    if (
        "USERNAME_CLASSIC_TIMEOUT = 20" not in worker
        or '"--top-sites", "300"' not in maigret
        or "R13.24 ADAPTIVE RELEVANCE" not in qml
    ):
        raise SystemExit("Install R13.24.1 (and its test compatibility fix) before R13.24.2.")


def main() -> int:
    parser = argparse.ArgumentParser(description="Install OSINTXZ R13.24.2 Balanced Relevance")
    ...
    backup = root / "storage" / "patch_backups" / f"r13_24_2_{stamp}"
    ...
    subprocess.run(
        [str(py), "-m", "py_compile", str(root / "app/application/adaptive_relevance.py")],
        cwd=root,
        check=True,
    )

    adaptive = (root / "app/application/adaptive_relevance.py").read_text(
        encoding="utf-8", errors="ignore"
    )
    required_markers = (
        "_weak_exact_relation",
        "No meaningful relation to the searched exact identifier",
        "Username-checking source reported a positive account observation",
        "Returned URL shares the searched host",
    )
    for marker in required_markers:
        if marker not in adaptive:
            raise SystemExit(f"R13.24.2 install verification failed: missing {marker}")

    print("R13.24.2 Balanced Relevance installed.")
